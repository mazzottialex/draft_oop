...
public class SimulatingMatchImplTest {
    private Team t1;
    private Team t2;
    private SimulatingMatch sm;
    ...
    public void setUp() throws FileNotFoundException, ClassNotFoundException, IOException {
        ...
        sm = new SimulatingMatchImpl(t1, t2);
        System.out.println(sm.result(0));
    }

    @Test
    public void testResult() throws FileNotFoundException, ClassNotFoundException, IOException {
        Map<Team, Integer> result = sm.result(0);
        assertTrue(result.get(t1) >= 0);
        assertTrue(result.get(t2) >= 0);
    }
}
